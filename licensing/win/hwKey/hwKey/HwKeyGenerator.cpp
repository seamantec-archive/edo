...
std::string HwKeyGenerator::getHwKey(){
	...
	returnString = mainboardHash.substr(0, 5) + "-" + mainboardHash.substr(5, 5);
	returnString += "-" + logicalDiskHash.substr(0, 5);
	returnString += "-" + memoryHash.substr(0, 5);
...
string HwKeyGenerator::getItemFromWMI(std::string table, std::string parameter){
	...
	while (pEnumerator)
	{
		...
		//returnString = (const char*)_bstr_t(V_BSTR(vtProp.bstrVal));
		if (vtProp.bstrVal){
			try
			{
				if (vtProp.vt != 1){
