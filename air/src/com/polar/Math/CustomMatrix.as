package com.polar.Math
{

import flash.geom.Matrix;
import flash.geom.Matrix3D;

public final class CustomMatrix extends Object
{
    ...
    public function CustomMatrix ( width:uint, height:uint, ... dat )
    {
    ...
    public function add ( mat:CustomMatrix ) : CustomMatrix
    {

        if ( mat.width != W || mat.height != H )
            throw new Error ( "Mismatched dimensions for adding.", 2 );

        var ret:CustomMatrix = new CustomMatrix ( W, H );
    ...
    public function sub ( mat:CustomMatrix ) : CustomMatrix
    {
    ...
    public function mulUniform ( val:Number ):CustomMatrix
    {
    ...
    public function mulMatrix ( mat:CustomMatrix ) : CustomMatrix
    {

        if ( W != mat.height )
            throw new Error ( "Mismatched dimensions for multiplication.", 4 );

        var ret:CustomMatrix = new CustomMatrix ( mat.width, H );
        var curr:Number;
    ...
    public function inverse () : CustomMatrix
    {
     ...
     */
    public function operationSupported ( operation:String, second:CustomMatrix = null ) : Boolean
    {
    ...
    public function toFlashMatrix () : Matrix
    {

        if ( ! operationSupported ( opFMX ) )
            throw new Error ( "Cannot convert to flash matrix. ( wrond dimentions )", 8 );

        var ret:Matrix = new Matrix ( data [ 0 ], data [ 1 ], data [ 3 ], data [ 4 ], data [ 2 ], data [ 5 ] );
        return ret;

    }

    //Makes the matrix a Flash 3D transform matrix.
    public function toFlashMatrix3D () : Matrix3D
    {

        if ( ! operationSupported ( opFM3 ) )
            throw new Error ( "Cannot convert to flash matrix. ( wrond dimentions )", 9 );

        var ret:Matrix3D = new Matrix3D ( data );
        return ret;

    }

    //Generates an identity matrix of a specific size.
    public static function generateIdentity ( size:uint ) : CustomMatrix
    {

        var ret:CustomMatrix = new CustomMatrix ( size, size );
        var i:uint = 0;
    ...
    public static function fromFlashMatrix ( mat:Matrix ) : CustomMatrix
    {

        return new CustomMatrix ( 3, 3, mat.a, mat.b, mat.tx, mat.c, mat.d, mat.ty, 0, 0, 1 );

    }

    //Creates a matrix from a flash 3D transform matrix.
    public static function fromFlashMatrix3D ( mat:Matrix3D ) : CustomMatrix
    {

        return new CustomMatrix ( 4, 4, mat.rawData );
