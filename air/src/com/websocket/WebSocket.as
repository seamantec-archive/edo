...
public class WebSocket extends EventDispatcher {
    ...
    private var connectedTimeout:int = 10000;
    ...
    private var _reconnectEnabled:Boolean = true;

    private var lastMessageTime:Number = 0;
    ...
    private function init():void {
        ...
        connectedTimer = new Timer(connectedTimeout);
        connectedTimer.addEventListener(TimerEvent.TIMER, connectedTimer_timerHandler);
...
//			}
    ...
    public function setLastMessageTime():void {
        lastMessageTime = getTimer();
    }

    private function connectedTimer_timerHandler(event:TimerEvent):void {
        if (lastMessageTime != 0 && getTimer() - lastMessageTime > 30000) {
            if (socket.connected) {
                close(false);
                return;
            }
...
//                this.dispatchEvent(new Event('CONNECTING'));
    ...
    private function onNetwork_ChangeHandler(event:Event):void {
        var networkInfo:NetworkInfo = NetworkInfo.networkInfo;
        var interfaces:Vector.<NetworkInterface> = networkInfo.findInterfaces();
        var hasAnyActive:Boolean = false;
        ...
        if (hasAnyActive && !socket.connected && _reconnectEnabled) {
            connect();
        } else if (!hasAnyActive && socket.connected) {
            close(false)
        }
    }
}
